...
def get_match_info(match_soup, match_url):
    ...
    competition = (
        match_soup.select("div.spielername-profil")[0]
        .select("h2")[0]
        .get_text()
        .strip()
    )
    ...
    home_team = (
        match_soup.select("div.sb-team.sb-heim.show-for-small")[0]
        .select("img")[0]["alt"]
        .lower()
    )
    home_team_id = (
    ...
    away_team = (
        match_soup.select("div.sb-team.sb-gast.show-for-small")[0]
        .select("img")[0]["alt"]
        .lower()
    )
    away_team_id = (
    ...
    if "Behind Closed Doors" in core_info.select("p.sb-zusatzinfos")[0].get_text():
        ...
        attendance = int(
            core_info.select("p.sb-zusatzinfos")[0]
            .select("strong")[0]
            .get_text()
            .replace("Attendance: ", "")
            .strip()
    ...
    try:
        match_date_raw = (
            core_info.select("p.sb-datum.hide-for-small")[0]
            .select("a")[1]
            .get_text()
            .strip()[5:]
        )
    ...
    home_goals, away_goals = map(
        int,
        core_info.select("div.ergebnis-wrap")[0]
        .get_text()
        .strip()
        .split("(")[0]
        .split(":"),
    ...
    home_ht_goals, away_ht_goals = map(
        int,
        core_info.select("div.ergebnis-wrap")[0]
        .get_text()
        .strip()
    ...
    home_formation = (
        match_soup.select(
            "div.large-7.aufstellung-vereinsseite.columns.small-12.unterueberschrift.aufstellung-unterueberschrift"
        )[0]
        .get_text()
        .replace("Starting Line-up: ", "")
        .strip()
    )
    away_formation = (
        match_soup.select(
...
def get_match_data(match_soup, match_info, match_url):
    ...
    if match_soup.find("div", {"id": "sb-tore"}) != None:
        for li in match_soup.find("div", {"id": "sb-tore"}).select("li"):
            ...
            player_id_list.append(
                li.select("div.sb-aktion-aktion")[0].select("a")[0]["id"]
            )
            player_name_list.append(
                li.select("div.sb-aktion-aktion")[0].select("a")[0]["title"]
            )
            ...
            home_score, away_score = map(
                int,
                li.select("div.sb-aktion-spielstand")[0].get_text().strip().split(":"),
            )
            ...
            for item in li.select("div.sb-aktion-aktion"):
                if len(item.get_text().split("Assist: ")) == 2:
                    ...
                    assisting_player_name_list.append(
                        li.select("a.wichtig")[1]["title"]
                    )
            ...
            ## get minutes
            bgPosCols, bgPosRows = map(
                int,
                li.select("span.sb-sprite-uhr-klein")[0]["style"]
            ...
            extra_mins = (
                0
                if li.select("span.sb-sprite-uhr-klein")[0]
                .get_text()
                .replace("+", "")
                .strip()
                == ""
                else int(
                    li.select("span.sb-sprite-uhr-klein")[0]
                    .get_text()
                    .replace("+", "")
                    .strip()
                )
            ...
            if len(goal_text.split(",")) < 3:
                ...
                shot_type = (
                    li.select("div.sb-aktion-aktion")[0]
                    .get_text()
                    .split(",")[1]
                    .strip()
                )
    ...
    if match_soup.find("div", {"id": "sb-wechsel"}) != None:
        for li in match_soup.find("div", {"id": "sb-wechsel"}).select("li"):
            ...
            subbing_team = (
                ...
                if li["class"][0] == "sb-aktion-heim"
                else match_info["away_team"]
            )

            try:
                sub_reason = (
                    li.select("div.sb-aktion-spielstand.hide-for-small")[0]
                    .select("span")[0]["title"]
                    .lower()
                )
            except:
            ...
            player_name = (
                li.select("div.sb-aktion-spielerbild")[0]
                .select("img")[0]["alt"]
                .lower()
            )
            player_id = (
                li.select("div.sb-aktion-spielerbild")[0].select("a")[0]["id"].lower()
            )

            player_name_in = (
                li.select("div.sb-aktion-spielerbild")[1]
                .select("img")[0]["alt"]
                .lower()
            )
            player_id_in = (
                li.select("div.sb-aktion-spielerbild")[1].select("a")[0]["id"].lower()
            )
    ...
    if match_soup.find("div", {"id": "sb-karten"}) != None:
        for li in match_soup.find("div", {"id": "sb-karten"}).select("li"):
            ...
            if "Yellow card" in li.select("div.sb-aktion-aktion")[0].get_text().strip():
                ...
                "Second yellow"
                in li.select("div.sb-aktion-aktion")[0].get_text().strip()
            ):
    ...
    for row in lineup_soup.select("table.items")[0].select("tr")[0::3]:
        ...
        age_list.append(
            int(row.get_text().strip().split("(")[1].strip().split(" years old")[0])
        )
        started_list.append(1)
        team_list.append(match_info["home_team"])
        position_list.append(
            row.get_text().strip().split(")")[1].split(",")[0].strip().lower()
        )
    ...
    # find max minutes
    max_minutes = (
        max(goals_data.minute.max(), card_data.minute.max(), subs_data.minute.max()) + 1
    )

    for i in range(len(lineup_data)):
        row = lineup_data.iloc[i]
        if row.started == 1:
            if row.player_name in list(
                subs_data[subs_data["sub_type"] == "out"]["player_name"]
            ):
                mins_played_list.append(
                    int(
                        subs_data[subs_data["player_name"] == row.player_name]["minute"]
                    )
                )
            ...
            if row.player_name in list(
                subs_data[subs_data["sub_type"] == "in"]["player_name"]
            ):
                mins_played_list.append(
                    max_minutes
                    - int(
...
def match_pull(
    ...
    if lineups:
        ...
        # csv output
            ...
            pd.DataFrame.from_dict(match_data["lineup_data"]).to_csv(
                (data_folder + str(match_info["match_id"]) + "_lineups.csv"),
                index=False,
            )
    ...
    if goals:
        ...
        # csv output
            ...
            pd.DataFrame.from_dict(match_data["goals_data"]).to_csv(
                (data_folder + str(match_info["match_id"]) + "_goals.csv"), index=False
            )
    ...
    if cards:
        ...
        # csv output
            ...
            pd.DataFrame.from_dict(match_data["card_data"]).to_csv(
                (data_folder + str(match_info["match_id"]) + "_cards.csv"), index=False
            )
    ...
    if subs:
        ...
        # csv output
            ...
            pd.DataFrame.from_dict(match_data["subs_data"]).to_csv(
                (data_folder + str(match_info["match_id"]) + "_subs.csv"), index=False
            )
    ...
    if info:
        ...
        # csv output
            ...
            pd.DataFrame.from_dict(match_info, orient="index").transpose().to_csv(
                (data_folder + str(match_info["match_id"]) + "_info.csv"), index=False
            )
