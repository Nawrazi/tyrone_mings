...
def get_league_transfers_summary(transfers_url, season="2019"):
    ...
    tranfser_overview["season"] = (
        soup.select("div.table-header")[0].get_text().replace("Transfers ", "").strip()
    )
    tranfser_overview["country"] = (
        soup.select("div.flagge")[0].select("img")[0]["alt"].lower()
    )
